-- SUPABASE SCHEMA DESIGN: The Survivor Packet
...
CREATE TABLE profiles (
  ...
  affiliate_id uuid, -- Linked if role is 'professional'
...
CREATE TABLE affiliate_referrals (
  ...
  owner_id uuid REFERENCES profiles(id), -- The professional who owns this link
  created_at timestamp with time zone DEFAULT now()
);

-- Add foreign key to profiles after affiliate_referrals is created
ALTER TABLE profiles ADD CONSTRAINT fk_profiles_affiliate FOREIGN KEY (affiliate_id) REFERENCES affiliate_referrals(id);

-- 19. Affiliate Conversions
CREATE TABLE affiliate_conversions (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  affiliate_referral_id uuid REFERENCES affiliate_referrals(id) ON DELETE SET NULL,
  purchase_id uuid, -- Added to link to purchases
  packet_id uuid REFERENCES packets(id) ON DELETE CASCADE,
...
-- 24. Section Records View (for global search/overview)
...
-- Helper function to check if user is a member of a packet
...
-- Profiles: Users can read/update their own profile. Admin can see all.
...
-- Packets: Members can view, owners can update. Admin can see all.
...
-- Packet Members: Members can view, owners can manage. Admin can see all.
CREATE POLICY "Members can view packet members" ON packet_members FOR SELECT USING (is_packet_member(packet_id) OR is_admin());
CREATE POLICY "Owners can manage packet members" ON packet_members FOR ALL USING (
  EXISTS (SELECT 1 FROM packets WHERE id = packet_id AND owner_user_id = auth.uid()) OR is_admin()
);

-- General Records (Family, Real Estate, etc.)
-- Members can view/manage records in their packets. Admin can see all.
CREATE POLICY "Members can view family_members" ON family_members FOR SELECT USING (is_packet_member(packet_id) OR is_admin());
CREATE POLICY "Members can manage family_members" ON family_members FOR ALL USING (is_packet_member(packet_id) OR is_admin());
...
CREATE POLICY "Members can manage real_estate_records" ON real_estate_records FOR ALL USING (is_packet_member(packet_id) OR is_admin());
...
CREATE POLICY "Members can manage banking_records" ON banking_records FOR ALL USING (is_packet_member(packet_id) OR is_admin());
...
CREATE POLICY "Members can manage retirement_records" ON retirement_records FOR ALL USING (is_packet_member(packet_id) OR is_admin());
...
CREATE POLICY "Members can manage vehicle_records" ON vehicle_records FOR ALL USING (is_packet_member(packet_id) OR is_admin());
...
CREATE POLICY "Members can manage advisor_records" ON advisor_records FOR ALL USING (is_packet_member(packet_id) OR is_admin());
...
CREATE POLICY "Members can manage password_records" ON password_records FOR ALL USING (is_packet_member(packet_id) OR is_admin());
...
CREATE POLICY "Members can manage personal_property_records" ON personal_property_records FOR ALL USING (is_packet_member(packet_id) OR is_admin());

CREATE POLICY "Members can view pet_records" ON pet_records FOR SELECT USING (is_packet_member(packet_id) OR is_admin());
CREATE POLICY "Members can manage pet_records" ON pet_records FOR ALL USING (is_packet_member(packet_id) OR is_admin());
...
CREATE POLICY "Members can manage funeral_records" ON funeral_records FOR ALL USING (is_packet_member(packet_id) OR is_admin());

CREATE POLICY "Members can view info_records" ON info_records FOR SELECT USING (is_packet_member(packet_id) OR is_admin());
CREATE POLICY "Members can manage info_records" ON info_records FOR ALL USING (is_packet_member(packet_id) OR is_admin());
...
CREATE POLICY "Members can manage medical_records" ON medical_records FOR ALL USING (is_packet_member(packet_id) OR is_admin());
...
CREATE POLICY "Members can manage section_completion" ON section_completion FOR ALL USING (is_packet_member(packet_id) OR is_admin());

-- Private Items: Stricter rules. Admin can see all (for emergency support).
CREATE POLICY "Private items access" ON private_items FOR SELECT USING (
...
CREATE POLICY "Private items management" ON private_items FOR ALL USING (owner_user_id = auth.uid() OR is_admin());

-- Documents
CREATE POLICY "Documents access" ON documents FOR SELECT USING (is_packet_member(packet_id) OR is_admin());
CREATE POLICY "Documents management" ON documents FOR ALL USING (is_packet_member(packet_id) OR is_admin());

-- Affiliate Referrals: Public read (to check code), Admin manage, Professional read own
...
CREATE POLICY "Admin manage referrals" ON affiliate_referrals FOR ALL USING (is_admin());

-- Affiliate Conversions: Members can view their own, Admin manage, Professional view own
...
CREATE POLICY "Admin manage conversions" ON affiliate_conversions FOR ALL USING (is_admin());
...
CREATE POLICY "Admin manage invites" ON partner_invites FOR ALL USING (is_admin());
...
CREATE POLICY "Admin manage billing" ON pricing_plans FOR ALL USING (is_admin());
CREATE POLICY "Admin manage billing profiles" ON customer_billing_profiles FOR ALL USING (is_admin());
CREATE POLICY "Admin manage purchases" ON purchases FOR ALL USING (is_admin());

-- STORAGE BUCKETS (Manual setup in Supabase UI, but policies here)
-- Buckets: packet-documents, packet-private-documents, packet-photos

-- Policy for packet-documents
-- (Assuming bucket name is 'packet-documents')
-- auth.uid() must be a member of the packet_id in the path
-- Path convention: {packet_id}/{section_key}/{record_id}/{filename}

-- Note: Storage policies are complex and often require a custom function or RPC to check membership efficiently.
