...
export default function CorporateAccept() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data, error } = await supabase
        ...
        .select("*, corporate_accounts(company_name, feature_tier)")
  ...
  const handleAccept = async () => {
    if (!user) {
      navigate(`/?redirect=/corporate/accept/${token}`);
      return;
    }
    setAccepting(true);
    try {
      ...
      toast.success("Your Survivor Packet seat is active!");
      navigate("/");
    } catch (err: any) {
  ...
  const company = seat.corporate_accounts?.company_name || "your company";

  return (
    <div className="min-h-screen flex items-center justify-center bg-stone-50 px-4">
      <div className="max-w-md w-full bg-white rounded-3xl border border-stone-100 shadow-sm p-8">
        <Building2 className="mx-auto text-navy-muted mb-3" size={40} />
        <h1 className="text-2xl font-serif font-bold text-navy-muted text-center">
          {company} has provided you a Survivor Packet
        </h1>
        <p className="text-sm text-stone-500 text-center mt-2">
          Accept to activate a fully independent personal account. Your data stays yours — even if you leave the company.
        </p>
        <div className="bg-stone-50 rounded-xl p-4 mt-6 text-sm">
          <div className="flex justify-between"><span className="text-stone-500">Email</span><span className="font-bold text-navy-muted">{seat.invited_email}</span></div>
          <div className="flex justify-between mt-1"><span className="text-stone-500">Tier</span><span className="font-bold text-navy-muted capitalize">{seat.corporate_accounts?.feature_tier} Corporate</span></div>
        </div>
        <p className="text-xs text-stone-500 mt-4 text-center">
          100% private. {company} cannot view your packet contents.
        </p>
        <button
        ...
        >
          ...
          {user ? "Accept seat" : "Sign in to accept"}
        </button>
      </div>
    </div>
  );
